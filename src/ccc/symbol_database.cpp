	...
	std::string name, Address address, SymbolSourceHandle source, const Module* module_symbol)
{
	...
	do {
		...
		CCC_CHECK(handle != NULL_SYMBOL_HANDLE, "Ran out of handles to use for %s symbols.", SymbolType::NAME);
...
template <typename SymbolType>
Result<SymbolType*> SymbolList<SymbolType>::create_symbol(std::string name,
	...
	if constexpr (SymbolType::FLAGS & NAME_NEEDS_DEMANGLING) {
		if ((importer_flags & DONT_DEMANGLE_NAMES) == 0 && demangler.cplus_demangle) {
			...
			if (importer_flags & DEMANGLE_PARAMETERS)
				demangler_flags |= DMGL_PARAMS;
			if (importer_flags & DEMANGLE_RETURN_TYPE)
				demangler_flags |= DMGL_RET_POSTFIX;
...
template <typename SymbolType>
void SymbolList<SymbolType>::mark_symbols_from_source_for_destruction(
	SymbolSourceHandle source, SymbolDatabase* database)
{
...
template <typename SymbolType>
void SymbolList<SymbolType>::mark_symbols_from_module_for_destruction(
	ModuleHandle module_handle, SymbolDatabase* database)
{
	for (SymbolType& symbol : m_symbols) {
...
template <typename SymbolType>
std::atomic<RawSymbolHandle> SymbolList<SymbolType>::m_next_handle = 0;
	...
	std::optional<std::vector<ParameterVariableHandle>> parameter_variables, SymbolDatabase& database)
{
	if (m_parameter_variables.has_value()) {
		for (ParameterVariableHandle parameter_variable_handle : *m_parameter_variables) {
			ParameterVariable* parameter_variable = database.parameter_variables.symbol_from_handle(
				parameter_variable_handle);
...
Result<DataType*> SymbolDatabase::create_data_type_if_unique(std::unique_ptr<ast::Node> node,
...
MultiSymbolHandle::MultiSymbolHandle(SymbolDescriptor descriptor, RawSymbolHandle handle)
...
RawSymbolHandle MultiSymbolHandle::handle() const
{
	return m_handle;
}
...
Symbol* MultiSymbolHandle::lookup_symbol(SymbolDatabase& database)
{
	...
	switch (m_descriptor) {
#define CCC_X(SymbolType, symbol_list)                                                     \
	case SymbolType::DESCRIPTOR: return database.symbol_list.symbol_from_handle(m_handle);
		CCC_FOR_EACH_SYMBOL_TYPE_DO_X
#undef CCC_X
	}
...
bool MultiSymbolHandle::is_flag_set(SymbolFlag flag) const
{
	if (m_handle != NULL_SYMBOL_HANDLE) {
		switch (m_descriptor) {
#define CCC_X(SymbolType, symbol_list)                            \
	case SymbolType::DESCRIPTOR: return SymbolType::FLAGS & flag;
			CCC_FOR_EACH_SYMBOL_TYPE_DO_X
#undef CCC_X
		}
	}
...
bool MultiSymbolHandle::move_symbol(Address new_address, SymbolDatabase& database) const
{
	if (m_handle != NULL_SYMBOL_HANDLE) {
		switch (m_descriptor) {
#define CCC_X(SymbolType, symbol_list)                                                           \
	case SymbolType::DESCRIPTOR: return database.symbol_list.move_symbol(m_handle, new_address);
			CCC_FOR_EACH_SYMBOL_TYPE_DO_X
#undef CCC_X
		}
	}
...
bool MultiSymbolHandle::rename_symbol(std::string new_name, SymbolDatabase& database) const
{
	if (m_handle != NULL_SYMBOL_HANDLE) {
		switch (m_descriptor) {
#define CCC_X(SymbolType, symbol_list)                                                                     \
	case SymbolType::DESCRIPTOR: return database.symbol_list.rename_symbol(m_handle, std::move(new_name));
			CCC_FOR_EACH_SYMBOL_TYPE_DO_X
#undef CCC_X
		}
	}
