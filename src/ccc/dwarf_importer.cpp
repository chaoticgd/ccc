...
SymbolTableImporter::SymbolTableImporter(SymbolDatabase& database,
...
Result<std::vector<OverlayInfo>> enumerate_overlays(const SectionReader& dwarf)
{
	...
	while (die.has_value()) {
		if (die->tag() == TAG_overlay) {
			Value overlay_id;
			Value overlay_name;
			Result<void> attribute_result = die->scan_attributes(overlay_attributes, {&overlay_id, &overlay_name});
			CCC_RETURN_IF_ERROR(attribute_result);
