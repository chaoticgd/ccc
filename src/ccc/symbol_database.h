...
struct SymbolHandle
{
	...
	SymbolHandle() {}
	SymbolHandle(RawSymbolHandle v)
		: value(v)
	{
	}
...
public:
	...
	std::vector<SymbolType*> optional_symbols_from_handles(
		const std::optional<std::vector<SymbolHandle<SymbolType>>>& handles);
	std::vector<const SymbolType*> optional_symbols_from_handles(
	...
	Result<SymbolType*> create_symbol(std::string name,
...
public:
	...
	Result<DataType*> create_data_type_if_unique(std::unique_ptr<ast::Node> node,
