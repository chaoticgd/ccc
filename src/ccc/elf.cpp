...
Result<ElfFile> ElfFile::parse(std::vector<u8> image)
{
	...
	for (u32 i = 0; i < header->shnum; i++) {
		...
		std::optional<std::string_view> name = get_string(
			elf.image, shstr_section_header->offset + section_header->name);
	...
	SymbolDatabase& database, const SymbolGroup& group, u32 importer_flags, const DemanglerFunctions& demangler) const
{
	...
	for (const ElfSection& known_section : sections) {
		...
		for (const ElfSection& symbol_section : sections) {
			bool is_symbol_section = symbol_section.name.size() > known_section.name.size()
	...
	SymbolDatabase& database, const SymbolGroup& group, u32 importer_flags, const DemanglerFunctions& demangler) const
{
	...
	for (const ElfSection& section : sections) {
		...
		if (link_once->is_text) {
			...
			Result<Function*> function = database.functions.create_symbol(std::move(link_once->symbol_name),
				group.source, group.module_symbol, address, importer_flags, demangler);
			CCC_RETURN_IF_ERROR(function);
		} else {
			...
			Result<GlobalVariable*> global_variable = database.global_variables.create_symbol(
				std::move(link_once->symbol_name), group.source, group.module_symbol, address, importer_flags,
				demangler);
			CCC_RETURN_IF_ERROR(global_variable);
...
{
	std::optional<std::span<const u8>> data = get_subspan(std::span(image), section.header.offset, section.header.size);
