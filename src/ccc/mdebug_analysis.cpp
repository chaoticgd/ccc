...
{
	Result<GlobalVariable*> global = m_database.global_variables.create_symbol(mangled_name, m_context.group.source,
...
Result<void> LocalSymbolTableAnalyser::function(const char* mangled_name, const StabsType& return_type, Address address)
{
	if (!m_current_function || strcmp(mangled_name, m_current_function->mangled_name().c_str()) != 0) {
		...
		bool no_module_base_address = m_context.group.module_symbol
			&& m_context.group.module_symbol->address().get_or_zero() == 0;
	...
	Result<std::unique_ptr<ast::Node>> node = stabs_type_to_ast(
		return_type, nullptr, m_stabs_to_ast_state, 0, true, true);
	CCC_RETURN_IF_ERROR(node);
	...
	const char* name, const StabsType& type, u32 value, StabsSymbolDescriptor desc, SymbolClass sclass)
{
	...
	if (desc == StabsSymbolDescriptor::STATIC_LOCAL_VARIABLE) {
		...
		std::optional<GlobalStorageLocation> location_opt = symbol_class_to_global_variable_location(sclass);
		CCC_CHECK(location_opt.has_value(), "Invalid static local variable location %s.", symbol_class(sclass));
...
{
	CCC_CHECK(
		m_state != IN_FUNCTION_BEGINNING, "Unexpected end of symbol table for '%s'.", m_source_file.name().c_str());
