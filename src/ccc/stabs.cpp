...
Result<StabsSymbol> parse_stabs_symbol(const char*& input)
{
	...
	CCC_CHECK(
		validate_symbol_descriptor(symbol.descriptor), "Invalid symbol descriptor '%c'.", (char) symbol.descriptor);
	CCC_CHECK(*input != '\0', "Unexpected end of input.");
	if (*input == 't') {
		input++;
	}
	...
	bool is_function = symbol.descriptor == StabsSymbolDescriptor::LOCAL_FUNCTION
		|| symbol.descriptor == StabsSymbolDescriptor::GLOBAL_FUNCTION;
	if (is_function && input[0] == ',') {
		input++;
		while (*input != ',' && *input != '\0')
			input++; // enclosing function
...
static bool validate_symbol_descriptor(StabsSymbolDescriptor descriptor)
{
	...
	switch (descriptor) {
		...
		case StabsSymbolDescriptor::REFERENCE_PARAMETER_V: valid = true; break;
...
static Result<std::unique_ptr<StabsType>> parse_stabs_type(const char*& input)
{
	...
	switch (descriptor) {
		...
		default: {
			return CCC_FAILURE("Invalid type descriptor '%c' (%02x).", (u32) descriptor, (u32) descriptor);
		}
	}
	...
	while (*input != '\0') {
		...
		while (*input != '\0') {
			...
			switch (modifiers) {
				...
				case '?':
				case '.': break;
				default: return CCC_FAILURE("Invalid member function modifiers.");
			}
			...
			switch (flag) {
				...
				default: return CCC_FAILURE("Invalid member function type.");
			}
...
STABS_DEBUG(
	...
	static void print_field(const StabsStructOrUnionType::Field& field) {
		printf("\t%04x %04x %04x %04x %s\n", field.offset_bits / 8, field.size_bits / 8, field.offset_bits,
			field.size_bits, field.name.c_str());
	}
